(() => {
    ...
    (() => {
        ...
        for (let i = 0; i < burgerButtons.length; i++) {
            const burgerButton = burgerButtons[i];
            burgerButton.addEventListener("click", (function(e) {
                burger__icon.classList.toggle("menu-open");
        ...
        const menuItems = [ {
            ...
            submenu: [ {
                text: "Possibilities",
                href: "#",
                img: "../img/icon-solution-01.svg",
                subText: "Accept payments globally with our worldwide card processing solution covering six continents."
            }, {
                text: "Protection",
                href: "#",
                img: "../img/icon-solution-02.svg",
                subText: "Our solution comes with pre-integrated fraud &amp; chargeback prevention tools from industry-leading suppliers"
            }, {
                text: "PSP & Fintech",
                href: "#",
                img: "../img/icon-solution-03.svg",
                subText: "Smart routing instantly assigns and directs your deposit to the most suitable acquirer, reducing false declines."
            }, {
                text: "Business & e-com",
                href: "#",
                img: "../img/icon-solution-04.svg",
                subText: "Optimise your payment process and accept transactions online."
            } ]
        }, {
            text: "About us",
            href: "#"
        ...
        for (let index = 0; index < menu__lists.length; index++) {
            ...
            for (let i = 0; i < menuItems.length; i++) {
                ...
                if (item.submenu) {
                    const submenuBody = document.createElement("div");
                    submenuBody.classList.add("menu__submenu-body");
                    li.append(submenuBody);
                    const submenuList = document.createElement("ul");
                    submenuList.classList.add("menu__submenu-list");
                    submenuBody.append(submenuList);
                    if (item.submenu) for (let ind = 0; ind < item.submenu.length; ind++) {
                        ...
                        submenuText.classList.add("submenu__text");
                        submenuText.innerText = submenu_Item.subText;
                        const submenuLink = document.createElement("a");
                        submenuLink.setAttribute("href", submenu_Item.href);
                        submenuLink.classList.add("submenu__link");
                        const submenuHead = document.createElement("div");
                        submenuHead.classList.add("submenu__head");
                        submenuItem.append(submenuLink);
                        submenuLink.append(submenuHead, submenuText);
                        const submenuIcon = document.createElement("img");
                        submenuIcon.setAttribute("src", submenu_Item.img);
                        submenuIcon.setAttribute("alt", "icon");
                        const submenuTitle = document.createElement("div");
                        submenuTitle.classList.add("submenu__title");
                        submenuTitle.innerText = submenu_Item.text;
                        submenuHead.append(submenuIcon, submenuTitle);
                    }
                }
            }
        }
        const homeItem = document.querySelectorAll(".header__menu .menu__item");
        for (let i = 0; i < homeItem.length; i++) if (homeItem[i].children[0].innerText === "Home") homeItem[i].remove();
        const menu__buttons = document.querySelectorAll(".header__menu .menu__item");
        document.querySelectorAll(".header__menu .menu__submenu-body");
        for (let i = 0; i < menu__buttons.length; i++) {
            menu__buttons[i].addEventListener("mouseenter", (function(e) {
                if (menu__buttons[i].children[1]) menu__buttons[i].children[1].classList.add("_active");
            }));
            if (menu__buttons[i].children[1]) menu__buttons[i].addEventListener("mouseleave", (function(e) {
                menu__buttons[i].children[1].classList.remove("_active");
            }));
        }
        const menList = document.querySelectorAll(".burger-body .menu__link");
        for (let i = 0; i < menuItems.length; i++) if (document.querySelector("." + menuItems[i].text.toLowerCase())) menList[i].classList.add("_active");
        for (let i = 0; i < menuItems.length; i++) {
            const item = menuItems[i];
            if (item.submenu) for (let ind = 0; ind < item.submenu.length; ind++) {
                const element = item.submenu[ind];
                let aLink = document.createElement("a");
                aLink.setAttribute("href", element.href);
                aLink.classList.add("footer-menu__link");
                aLink.innerText = element.text;
                footer_menu[1].append(aLink);
            } else {
                let aLink = document.createElement("a");
                aLink.setAttribute("href", item.href);
                aLink.classList.add("footer-menu__link");
                aLink.innerText = item.text;
                footer_menu[2].append(aLink);
            }
        }
        ...
        if (document.querySelector("[data-acc]")) {
            ...
            for (let i = 0; i < faq.length; i++) {
                const item = faq[i];
                accordion.innerHTML += `<div class="accordion__item"><div class="accordion__head"><div data-aos="zoom-in-up" class="accordion__title"><span>${i + 1}.</span>${item.question}</div><div data-aos="zoom-in-up" class="accordion__icon"></div></div><div class="accordion__content"><p data-aos="zoom-in-up" class="accordion__text">${item.text}</p></div></div>`;
            }
            const accordion__items = document.querySelectorAll(".accordion__item");
            if (accordion.getAttribute("data-acc-opened")) {
                let accordionNumber = accordion.getAttribute("data-acc-opened");
                accordion__items[accordionNumber].classList.add("_active");
            }
            for (let i = 0; i < accordion__items.length; i++) {
                const item = accordion__items[i];
                item.addEventListener("click", (function(e) {
                    if (accordion.getAttribute("data-acc") !== "closable") for (let index = 0; index < accordion__items.length; index++) {
                        const element = accordion__items[index];
        ...
        if (document.querySelector("[data-acc-2]")) {
            const accordion = document.querySelector("[data-acc-2]");
            const faq = [ {
                question: "S2S integration",
                text: "A customizable payment form is hosted on the gateway provider's server. The cardholder enters personal data on the merchant's registration page. When reaching the checkout page, the client is redirected to a 3rd party payment provider's form and enters their card details. After the payment is successfully made, they are redirected back to the merchant's website."
            }, {
                question: "H2H integration",
                text: "H2H integration. The cardholder submits their personal and card information on the merchant's website. The data is then securely transferred directly to the gateway server."
            }, {
                question: "Cashier/Platform provider",
                text: "Cashier/Platform provider. GumballPay is integrated with all major cashiers and platform providers and lets merchants manage their payments via a single access point."
            } ];
            for (let i = 0; i < faq.length; i++) {
                const item = faq[i];
                accordion.innerHTML += `<div data-tabs-title-2 class="accordion__item tabs__title"><div class="accordion__head"><div data-aos="zoom-in-up" class="accordion__title">${item.question}</div></div><div class="accordion__content"><p data-aos="zoom-in-up" class="accordion__text">${item.text}</p></div></div>`;
            }
            const accordion__items = document.querySelectorAll(".accordion__item");
            if (accordion.getAttribute("data-acc-opened")) {
                let accordionNumber = accordion.getAttribute("data-acc-opened");
                accordion__items[accordionNumber].classList.add("_active");
                accordion__items[accordionNumber].classList.add("_tab-active");
            }
            for (let i = 0; i < accordion__items.length; i++) {
                const item = accordion__items[i];
                item.addEventListener("click", (function(e) {
                    ...
                    item.classList.toggle("_active");
                    item.classList.toggle("_tab-active");
                }));
            }
        }
        if (document.querySelector("[data-tabs]")) {
            ...
            for (let i = 0; i < tabTitles.length; i++) {
                ...
                if (tabTitle.classList.contains("_tab-active")) {
                    tabContents[i].classList.add("_content-active");
                    tabContents[i].children[0].children[0].classList.add("open");
                }
                tabTitle.onclick = () => {
                    for (let ind = 0; ind < tabContents.length; ind++) {
                        ...
                        tabContents[i].children[0].children[0].classList.remove("open");
                    }
        ...
        if (document.querySelector("[data-tabs-2]")) {
            const tabTitles = document.querySelectorAll("[data-tabs-title-2]");
            const tabContents = document.querySelectorAll("[data-tabs-body-2]");
            for (let i = 0; i < tabTitles.length; i++) {
                const tabTitle = tabTitles[i];
                if (tabTitle.classList.contains("_tab-active")) {
                    tabContents[i].classList.add("_content-active");
        ...
        if (document.querySelector("[data-tabs-closed]")) {
            ...
            for (let i = 0; i < tabTitles.length; i++) {
                const tabTitle = tabTitles[i];
                if (tabTitle.classList.contains("_tab-active")) tabContents[i].classList.add("_opened");
                tabTitle.onclick = () => {
                    for (let ind = 0; ind < tabContents.length; ind++) {
                        const tabContent = tabContents[ind];
        ...
        function initSliders() {
            if (document.querySelector(".pay-offer__slider")) new Swiper(".pay-offer__slider", {
        ...
        const burgerSubMenuButton = document.querySelectorAll(".burger-body__menu button");
        const burgerSubMenubody = document.querySelectorAll(".burger-body__menu .menu__submenu-body");
        for (let i = 0; i < burgerSubMenuButton.length; i++) {
            const item = burgerSubMenuButton[i];
            item.addEventListener("click", (function(e) {
                _slideToggle(burgerSubMenubody[i]);
            }));
        }
        let isMobile = {
            Android: function() {
            ...
            if (wrapper && !isMobile.any()) {
                ...
                function mouseActions(e) {
                    if (e.type === "mouseout") cursor.style.opacity = 0; else if (e.type === "mousemove") {
                        ...
                        if (e.target.closest("button") || e.target.closest("a") || e.target.closest("input")) cursor.classList.add("_hover"); else cursor.classList.remove("_hover");
        ...
        if (document.querySelector(".header__link")) {
            ...
            for (let i = 0; i < header__link.length; i++) {
                const item = header__link[i];
                item.innerText = "Let's talk";
            }
        }
        const phoneNumberText = "+447312221374";
        const emailText = "info@akari-up.pro";
        if (document.querySelector(".number")) {
            const numbers = document.querySelectorAll(".number");
            for (let i = 0; i < numbers.length; i++) {
                const number = numbers[i];
                number.innerText = phoneNumberText;
                number.setAttribute("href", "tel:" + phoneNumberText);
            }
        }
        ...
        if (document.querySelector(".telegram")) {
            const telegrams = document.querySelectorAll(".telegram");
            for (let i = 0; i < telegrams.length; i++) {
                const telegram = telegrams[i];
                telegram.innerHTML = '<i class="fa fa-telegram"></i>';
            }
        }
        if (document.querySelector(".whatsapp")) {
            ...
            for (let i = 0; i < whatsapps.length; i++) {
                ...
                whatsapp.innerHTML = '<i class="fa fa-whatsapp"></i>';
            }
        }
        const stars = document.querySelectorAll(".stars");
        for (let ind = 0; ind < stars.length; ind++) {
            const item = stars[ind];
            for (let i = 0; i < 50; i++) {
                let star = document.createElement("span");
                star.classList.add("star");
                if (i % 2 == 0) star.classList.add("star-2"); else star.classList.add("star-1");
                item.append(star);
